        ...
        .exec(function(err, user) {
            ...
            Script.find()
                ...
                .exec(function(err, script_feed) {
                    Script.find({ class: { "$in": user.interests }, _id: { "$in": current_day == 0 ? user.day1_posts : user.day2_posts } })
                        .populate('actor')
                        .populate({
                            path: 'comments.actor',
                            populate: {
                        ...
                        }).exec(function(err, interest_posts) {
                            if (err) { return next(err); }

                            //Final array of all posts to go in the feed
                            let finalfeed = [];

                            //Concatenate interest_posts with script_feed to get user feedActions for them, reappend at end
                            script_feed = script_feed.concat(interest_posts);
                            interest_posts = [];
                            ...
                            while (script_feed.length || user_posts.length) {
                                ...
                                if (typeof script_feed[0] === 'undefined' ||
                                    ...
                                    if (feedIndex != -1) {
                                        ...
                                        // Check if post has been flagged or hidden: remove it from feed array (script_feed)
                                        if (user.feedAction[feedIndex].flagTime[0] || user.feedAction[feedIndex].hideTime[0]) {
                                        ...
                                        else if (script_feed[0].class != "normal") {
                                            interest_posts.push(script_feed[0]);
                                            script_feed.splice(0, 1);
                                        } else {
                            ...
                            const numAds = {
                                "var1": 5, //For every 10 posts, 5 posts are randomly replaced with an ad. Ad does not replace the first post.
                                "var2": 4, //For every 10 posts, 4 posts are randomly replaced with an ad. Ad does not replace the first post.
                                "var3": 2 //For every 10 posts, 2 posts are randomly replaced with an ad. Ad does not replace the first post.
                            }[user.group];
                            let count = 0;
                            ...
                            // for (const interestPostObject of interest_posts) {
                            //     console.log(spliceIndex * (count + 1) + count);
